...
exports.withdrawPost = async (req, res) => {
    ...
    await db.query('INSERT INTO withdraw SET id = ?, amount = ?, user_id = ?, upi_id = ?,name =? , IFSC =? , accountNum = ? ,created_at = ?', [uuidv4(), taxDeductedAmount, user_id, upi_id, name, IFSC, parseInt(accountNum), new Date()], (err, result) => {
        if (err) {
            console.log(err);
