...
exports.createDepositBackup = async (req, res) => {
    ...
    await db.query('INSERT INTO deposit SET id = ?, user_id = ?, reference_id = ?, amount = 0, verification = ?, created_at = ?', [uuidv4(), user_id, reference_id, amount, 'pending', new Date()], (err, result) => {
        if (err) {
            console.log(err);
